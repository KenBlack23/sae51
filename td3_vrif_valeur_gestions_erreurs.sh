...
if [[ "$OS" == "Linux" ]]; then
    ...
    echo "Le système est non pris en charge : $OS"
    exit 1
fi
    ...
    if [[ "$OS" == "Linux" ]]; then
        if pgrep -x "VirtualBox" > /dev/null; then
            ...
            if [[ "$choix" == "O" || "$choix" == "o" ]]; then
                echo "Lancement de VirtualBox en cours..."
                VirtualBox &
            else
...
function list_vms {
    ...
    while read -r vm; do
        ...
        ####################################### Obtenir l'état actuel de la machine ###############################################
        vm_state=$("$VBOXMANAGE" showvminfo "$vm_name" --machinereadable | grep "VMState=" | cut -d'=' -f2 | tr -d '"')

        ############################### Afficher les informations de l'état de la machine #########################################
        if [[ "$vm_state" == "running" ]]; then
            echo "État de la machine : Allumée"
        elif [[ "$vm_state" == "paused" ]]; then
            echo "État de la machine : En pause"
        elif [[ "$vm_state" == "poweroff" ]]; then
            echo "État de la machine : Éteinte"
        else
            echo "État de la machine : Inconnu ($vm_state)"
        fi
        ...
        if [[ -n "$last_start" ]]; then
            echo "Dernière connexion (changement d'état) : $last_start"
        else
...
function create_vm {
    while true; do
        ...
        if [[ -n "$existing_vm" ]]; then
            echo "Erreur : Une machine avec ce nom existe déjà. Veuillez saisir un nouveau nom."
            continue
        fi
    ...
    # Vérification de la taille de la RAM (entre 512 et 16384 MB)
    while true; do
        read -p "Taille de la RAM (en MB, entre 512 et 16384) : " RAM_SIZE
        if [[ ! "$RAM_SIZE" =~ ^[0-9]+$ ]]; then
            echo "Erreur : La taille de la RAM doit être un nombre entier."
        elif [[ "$RAM_SIZE" -lt 512 || "$RAM_SIZE" -gt 16384 ]]; then
            echo "Erreur : La RAM doit être comprise entre 512 et 16384 MB."
        else
            break
        fi
    done
    ...
    # Vérification de la taille du disque (entre 1 et 1024 GB)
    while true; do
        read -p "Taille du disque dur (en Go, entre 1 et 1024) : " HDD_SIZE
        if [[ ! "$HDD_SIZE" =~ ^[0-9]+$ ]]; then
            echo "Erreur : La taille du disque dur doit être un nombre entier."
        elif [[ "$HDD_SIZE" -lt 1 || "$HDD_SIZE" -gt 1024 ]]; then
            echo "Erreur : La taille du disque doit être comprise entre 1 et 1024 Go."
        else
            break
        fi
    done
    ...
    # Vérification du nombre de CPU (entre 1 et 8)
    while true; do
        read -p "Nombre de CPU (entre 1 et 8) : " CPU_COUNT
        if [[ ! "$CPU_COUNT" =~ ^[0-9]+$ ]]; then
            echo "Erreur : Le nombre de CPU doit être un nombre entier."
        elif [[ "$CPU_COUNT" -lt 1 || "$CPU_COUNT" -gt 8 ]]; then
            echo "Erreur : Le nombre de CPU doit être compris entre 1 et 8."
        else
            break
        fi
    done
    ...
    if ! "$VBOXMANAGE" createvm --name "$VM_NAME" --ostype "Debian_64" --register; then
        echo "Erreur : Échec de la création de la machine virtuelle."
        return
    fi

    "$VBOXMANAGE" modifyvm "$VM_NAME" --memory "$RAM_SIZE" --cpus "$CPU_COUNT" --nic1 nat --boot1 net
    if ! "$VBOXMANAGE" createmedium disk --filename "$VM_NAME"_disk.vdi --size "$HDD_SIZE"; then
        echo "Erreur : Échec de la création du disque dur virtuel."
        return
    fi
...
# Fonction pour supprimer une VM avec gestion des erreurs
function delete_vm {
    read -p "Entrez le nom de la machine à supprimer : " VM_NAME
    if ! "$VBOXMANAGE" unregistervm "$VM_NAME" --delete; then
        echo "Erreur : Échec de la suppression de la machine $VM_NAME."
    else
...
# Fonction pour démarrer une VM avec gestion des erreurs
function start_vm {
    read -p "Entrez le nom de la machine à démarrer : " VM_NAME
    if ! "$VBOXMANAGE" startvm "$VM_NAME"; then
        echo "Erreur : Échec du démarrage de la machine $VM_NAME."
    else
        echo "VM $VM_NAME démarrée avec succès."
    fi
}

############################################# Fonction pour arrêter une VM ##################################################
# Fonction pour arrêter une VM avec gestion des erreurs
function stop_vm {
    read -p "Entrez le nom de la machine à arrêter : " VM_NAME
    if ! "$VBOXMANAGE" controlvm "$VM_NAME" poweroff; then
        echo "Erreur : Échec de l'arrêt de la machine $VM_NAME."
    else
        echo "VM $VM_NAME arrêtée avec succès."
    fi
}
